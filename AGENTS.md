...
| `themes/learnlearnin/layouts/partials/` | `head.html`, `header.html`, `footer.html`, `share.html`, `menu.html`, `terms.html`, `head/css.html`, `head/js.html` |
...
- **`baseof.html`** — Root template with `<html>`, `<head>`, `<body>`, `<header>`, `<main>`, and `{{ partial "footer.html" }}`
...
- **No categories/tags** — navigation is purely through cross-links and DuckDuckGo site search
