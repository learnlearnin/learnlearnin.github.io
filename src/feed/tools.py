...
Read at """ + entry['link']
  entry['published'] = entry[
      'published'] if 'published' in entry else gettime()
  if "dirty" in entry:
...
def entrytoxml(entry):
  ...
  entrystring += '<author><name>' + entry['author'] + '</name><uri>' + entry[
      'authoruri'] + '</uri><email>' + entry['authoremail'] + '</email></author>'
  entrystring += '<content type="' + \
      entry['contenttype'] + '">' + entry['content'] + '</content>'
  entrystring += '</entry>'
  return entrystring
...
def getfiles(args):
  ...
  atompath = os.path.join(
      os.path.dirname(dirname), "files", filename.split('.')[0] + ".atom")
...
def telegramshare(message, cat="none"):
  ...
  for chat in config.tgchats:
    if cat == "none":
      ...
      try:
        if chat[1] == cat:
          # sends people with explicit tags the messages of their tags
          sendto += chat[0] + " "
      except IndexError:
        # sends people with no explicit tags all messages
        sendto += chat[0] + " "
